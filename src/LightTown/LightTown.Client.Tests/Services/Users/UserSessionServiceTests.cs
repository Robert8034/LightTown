...
namespace LightTown.Client.Tests.Services.Users
{
    public class UserSessionServiceTests
    {
        ...
        [Fact]
        public async Task LoadUserTest()
        {
            //ARRANGE
            ...
            Assert.Equal("TestUser", userSessionService.GetCurrentUser().Username);
        }
        ...
        public void SetCurrentUserTest()
        {
            ...
            //ACT
            userSessionService.SetCurrentUser(new User
            {
                Username = "TestUser"
            });

            //ASSERT
            Assert.NotNull(userSessionService.GetCurrentUser());
            Assert.Equal("TestUser", userSessionService.GetCurrentUser().Username);
        }
        
        /// <summary>
        /// Test whether TryLoadLocalUser returns the right value based on the cookie value it should get from the client.
        /// </summary>
        ...
        public async Task TryLoadLocalUserTest(string cookie, bool isValid)
        {
            //ARRANGE
            _ijsRuntimeMock.SetupIjsRuntimeMock<string>("getCookies", cookie);

            var userSessionService = new UserSessionService(_httpClient, _ijsRuntimeMock.Object, _navigationManagerMock.Object);

            //ACT
            bool result = await userSessionService.TryLoadLocalUser();
